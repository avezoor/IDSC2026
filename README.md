...
1. Load the dataset from the repository's `datasets/` directory and validate the expected recording structure, metadata, 12-lead coverage, 100 Hz sampling, and 12-second duration.
...
This section prepares the dependencies, the base configuration, and the dataset location validation logic.
...
## 3.3 Locate Dataset in `datasets/`

The pipeline currently expects the Brugada-HUCA dataset to be available at the repository path `datasets/` and validates the required files there before continuing. If you place the dataset elsewhere, update the path logic in `pipeline/initialization.py` accordingly.
